...
export default function DeleteBuyer({deleteBuyerModel , setUpdatePage , updatePage , singleBuyer}) {
    ...
    const deleteItem = async (id) => {
        ...
        try {
            const response = await fetch(`${GlobalApiState.DEV_BASE_LIVE}/api/buyer/delete_buyer/${singleBuyer._id}`, {
                method: 'DELETE'
            });
                                ...
                                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                    <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                        <p className="text-xl text-center text-gray-500">
                                            Are you sure you want to delete this Buyer ?</p>
                                    </div>
